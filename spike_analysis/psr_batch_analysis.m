function data = psr_batch_analysis(filenames,loadPath,savePathData,savePathFigs,cfg)
...
if (exist(fpath,'file') == 2) %% Load data
    ...
    for iProbe = 1:nProbes
        ...
        if (isfield(spikes,'stimoffest')); spikes.info.stimoffset = metadata.stimoffset(k,:);
        else,                              spikes.info.stimoffset = zeros(sum(k),2);
        end
        ...
        % Initialize arrays
        if (iProbe == 1)
            stimOffsets             = zeros(nStims,nProbes);
        ...
        spikes = psr_sst_cluster_classifier(spikes,parameters);
        ...
        if (length(metadata.stimulus) > 1)
            for iStim = 1:nStims
                ...
                spikesTrl.info.stimoffset = spikesTrl.info.stimoffset(iStim,:);
                stimOffsets(iStim,iProbe) = spikesTrl.info.stimoffset(1);
                
                %% Population
                
                spikesPop = spikesTrl;
                type    = [spikesPop.clusters.metrics.type];
                tf      = type >= 5;
                spikesPop.clusters.metrics(~tf) = [];
                ID      = [spikesPop.clusters.metrics.id];
                id_all  = [];
                nclusts = length(ID);
                ...
                spikesPop.clusters.metrics    = [];
                spikesPop.clusters.metrics.id = 1;
                
                %% Single units
                
                type = [spikesTrl.clusters.metrics.type];
                tf   = type >= 5; % Only full single-units
                spikesTrl.clusters.metrics(~tf) = [];
                ...
                ClusterIDsSingle = [spikesTrl.clusters.metrics.id];
...
if (cfg.plot_LFP)
    ...
    for iProbe = 1:nProbes
        ...
        for iStim = 1:nStims
            ...
            if (isnan(freqStim.powspctrm)); continue; end
            freqStim.std = squeeze(nanstd(freqStim.powspctrm,[],1)); % Standard deviation for every bin
            
            if (params.lfp.plot_mean || params.stims(iStim) == 0)
                nDims = ndims(freqStim.powspctrm);
                if (nDims == 4); freqStim.powspctrm = nanmean(freqStim.powspctrm,1); end
            end
            
            % Offset time axis
            
            freqStim.time = freqStim.time - stimOffsets(iStim,iProbe);
            ...
            inputs.method         = 'plot';
            inputs.cfg.xlim       = params.lfp.window / 1000;
            ...
            nTrials = size(freqStim.powspctrm,1);
            for iTrial = 1:nTrials
                ...
                inputs.freq           = freqStim;
                inputs.freq.powspctrm = inputs.freq.powspctrm(iTrial,:,:,:);
                inputs.freq.powspctrm = squeeze(inputs.freq.powspctrm);
                inputs.method         = 'plot';
                inputs.cfg.basetype   = params.lfp.base_type;
                inputs.cfg.xlim       = params.lfp.window / 1000;
                
                if (params.lfp.base); inputs.cfg.baseline = getLFPBaseline(freqStim,params);
                else,                 inputs.cfg.baseline = [];
                end
                ...
                titleStr = ['Probe = ' num2str(iProbe) ' , Stim: ' num2str(stim) 'V'];
                imageStr = [savePathLFP 'LFP-P' num2str(iProbe) '-V' num2str(stim)];
                
                if (params.lfp.plot_mean || params.stims(iStim) == 0)
                    suptitle(titleStr);
                    export_fig(imageStr);
                else
                    suptitle([titleStr ', T: ' num2str(iTrial)]);
                    export_fig([imageStr '-T' num2str(iTrial)]);
                end
            end
        end
    end
...
if (cfg.plot_amp) %% Amplitude dependence
    ...
    ampPopulation = amplitudeAnalysis(SpikeCountPopulationAll, params);
    ampSingle     = amplitudeAnalysis(SpikeCountSingleAll,     params);
    
    amplitudePlotting(ampPopulation, params, savePathAmpPop);
    amplitudePlotting(ampSingle,     params, savePathAmpSgl, ClusterIDsSingleAll);
    
end

if (cfg.plot_diff) %% Pre- and post-stimulus spiking difference
    ...
    for iStim = 1:nStims
        ...
        if (~isempty(FiringRate))
            ...
            for iClust = 1:nClusts
                ...
                off = params.stimOffset_2 - params.stimOffset_1;
                t1  = params.t_del(1);
                t2  = params.t_del(2);
                x1 = [t1,       t1,       t2,       t2];
                x2 = [t1 + off, t1 + off, t2 + off, t2 + off];
                y = [-yMax yMax yMax -yMax];
                patch(x1,y,'red','EdgeColor','none','FaceAlpha',0.2);
                patch(x2,y,'red','EdgeColor','none','FaceAlpha',0.2);
