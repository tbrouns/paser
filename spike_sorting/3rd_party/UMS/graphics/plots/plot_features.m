...
if has_miniclusters
    ...
    plot(zeros([1 num_spikes]), zeros([1 num_spikes]), '.' );
end
...
findobj(hax,'Type','line');
if figdata.show_outliers; item = findobj(hax,'Tag','image_with_outliers');
else                      item = findobj(hax,'Tag','image_without_outliers');
end
...
% update checkmark
if axdata.show_legend; legend('show'); show_leg = 'on';
else                   legend('hide'); show_leg = 'off';
end
...
for j = 1:length(hndls)
    ...
    elseif clus == 0
        indices = 0;
        outlier_loc = [outlier_loc j]; %#ok
    else
        indices = find(data.spikes.info.kmeans.assigns == clus);
    end
    x{j} = get_feature(data.spikes, indices, data.xchoice, data.xparam); %#ok
    y{j} = get_feature(data.spikes, indices, data.ychoice, data.yparam); %#ok
...
for j = 1:length(hndls)
    ...
    if clus == 0
        if data.show_outliers, visible = 'on';
        else                   visible = 'off';
        end
...
switch (feature),
    ...
    case 'PC', % (for principal components, we might need to compute the first time)
        if is_outliers; x = data_source.waveforms(which,:) * spikes.info.pca.v(:,param);
        else            x =      spikes.waveforms(which,:) * spikes.info.pca.v(:,param);
        end
    case 'Cluster',
        if is_outliers, x = zeros([1 length(which)]);
        else            x = spikes.assigns(which); 
        end
    case 'Minicluster',
        if is_outliers, x = zeros([1 length(which)]);
        else            x = spikes.info.kmeans.assigns(which); 
        end
    case 'Event Time',
...
function make_control(~, ~, controlaxis)
...
else  assigns = ones(size(spikes.unwrapped_times));
end
...
coords  = get(h,'Units'); set(h,'Units','pixel');
fig_pos = get(h,'Position');
ptr_pos = get(h,'CurrentPoint'); set(h,'Units',coords);
...
function listbox_callback(~,~,which)
...
if which == 'y' && isequal(choice,'PC'); defaultval = 2; 
else                                     defaultval = 1; 
end
...
function control_click(~,~,val,which)
...
function popup_deleted(~,~,which,ax)
data = get(ax,'UserData');
data.([which 'control']) = [];
set(ax,'UserData',data);
end
