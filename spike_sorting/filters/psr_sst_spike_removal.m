...
if (~isempty(id))
    ...
    if (id(1) < 1 || id(end) > nlength)
        disp('Error in "psr_sst_spike_removal": spike indices are out of bounds'); 
        return; 
    end
end
...
elseif (strcmp(method,'keep')) % keep only the input indices
    ...
    if (isfield(spikes,'features'));      spikes.features      = spikes.features   (:,id); end
