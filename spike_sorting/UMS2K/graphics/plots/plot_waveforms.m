...
% the total number of spikes and the number of refractory period violations
...
if data.colormode == 4 && length(unique(spikes.assigns(show))) == 1
    ...
    % this is a bit of a hack:  if we are plotting a subtree, want to make
    % color and name same as subtree tope node
elseif issubtree
    subclusterlist = unique(spikes.info.kmeans.assigns(show))';
    idx = find(ismember(spikes.info.tree(:,1),subclusterlist),1,'last');
    if isempty(idx); subclus = spikes.info.kmeans.assigns(show(1));
    else             subclus = spikes.info.tree(idx,1);
    end
...
function update_waveforms(~, ~, colormode, displaymode, ax)
...
if isequal(get(legend,'Visible'),'on'); legend('hide'); set(item,'Checked','off');
else                                    legend('show'); set(item,'Checked','on');
end

% plot to didsplay waveforms
function make_waveforms(~,data)
...
elseif displaymode == 3 % use 2D density plot
    cmap    = data.cmap;
    [n,x,y] = histxt(waveforms(:,:));
    h       = imagesc(x,y,n);
    colormap(cmap);
    set(gca,'Color', cmap(1,:) );
else % plot waveforms in groups
...
if displaymode == 2; set(l,'ButtonDownFcn', {@raise_band, l})
else                 set(l,'ButtonDownFcn', {@raise_me})
end
