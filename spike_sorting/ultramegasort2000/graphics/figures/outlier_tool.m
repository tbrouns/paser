...
if figdata.mode; tip = 'Save to workspace (s)';            icon = data.ims.im_save;
else             tip = 'Save to merge tool and close (s)'; icon = data.ims.im_save_and_close;
end
...
if isempty(filename_default); filename = 'spikes.mat';
else                          filename = [filename_default.pathname filename_default.filename];
end

% ask user where to save file
[FileName,PathName,~] = uiputfile('*.mat','Save Spikes object',filename);
...
if isempty(filename_default); filename = [pwd '\spikes.mat'];
else                          filename = [filename_default.pathname filename_default.filename];
end
...
if isempty(r_button), zoom_factor = 1; 
else                  zoom_factor = get(r_button,'UserData'); 
end
