...
fwin    = round((parameters.filter.fft.freq / Fs) * nlength); % Moving frequency window
...
while flag
    ...
    thresh = parameters.filter.fft.thresh * psr_mad(Y_win); % Calculate threshold
...
fwin = ceil((parameters.filter.fft.pad / Fs) * nlength); % Size of window to remove artifact
