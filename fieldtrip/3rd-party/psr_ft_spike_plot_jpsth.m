function [cfg] = psr_ft_spike_plot_jpsth(cfg, jpsth)
...
%   field JPSTH.avg. If cfg.barplot = 'psth', the field JPSTH.psth must be
%   present as well.
%
...
cfg = ft_checkopt(cfg,'barplot',     'char', {'psth','mean','sum','no'});
...
cfg = ft_checkconfig(cfg, 'allowed', {'channelcmb', 'barplot', 'latency', 'colorbar', 'colormap', 'interpolate', 'window', 'winlen', 'gaussvar'});
...
if isfield(jpsth,'jpsth'); dens = squeeze(jpsth.jpsth(         cmbindx(1,1), cmbindx(1,2),:,:)); % density
else,                      dens = squeeze(jpsth.shiftpredictor(cmbindx(1,1), cmbindx(1,2),:,:)); % density
end
...
switch cfg.latency
    case 'maxperiod'; cfg.latency = [minTime maxTime];
    case 'poststim';  cfg.latency = [0 maxTime];
    case 'prestim';   cfg.latency = [minTime 0];
end

% check whether the time window fits with the data
if (cfg.latency(1) < minTime); cfg.latency(1) = minTime; warning('Correcting begin latency of averaging window'); end
if (cfg.latency(2) > maxTime); cfg.latency(2) = maxTime; warning('Correcting end latency of averaging window');   end
...
if strcmp(cfg.colorbar,'yes'), pos(3) = 0.95 * pos(3); end

% plot the histogram if requested
if ~strcmp(cfg.barplot,'no')
    ...
    for iChan = 1:2
        ...
        switch cfg.barplot
            case 'psth'; bars = jpsth.psth(cmbindx(1,iChan),timeSel);
            case 'mean'; bars = mean(dens,   iChan);
            case 'sum';  bars =  sum(dens,[],iChan);
        end
        
        label = jpsth.label{cmbindx(1,iChan)};
        
        if iChan == 2; startPos = pos(1:2) + pos(3:4) .* [0.2 0.0] ; % shift the width 20%
        else,          startPos = pos(1:2) + pos(3:4) .* [0.0 0.2] ; % shift the height 20%
        end
        
        % and decrease the size of width and height
        if iChan == 2; sz = pos(3:4) .* [0.8 0.2]; 
        else,          sz = pos(3:4) .* [0.2 0.8]; 
        end
        
        ax(4 - iChan) = axes('Units', get(ax(1), 'Units'), 'Position', [startPos sz], 'Parent', get(ax(1), 'Parent'));
                    
        if iChan == 2; psthHdl(iChan) = bar (jpsth.time(timeSel),bars,'k'); % plot the psth under the jpsth
        else,          psthHdl(iChan) = barh(jpsth.time(timeSel),bars,'k'); % plot the psth left
        end
        
        if iChan == 2; set(ax(2),'YDir', 'reverse')
        else,          set(ax(3),'XDir', 'reverse')
        end
        
        if iChan == 2; ylabel(label)
        else,          xlabel(label)
        end
    end
    ...
    hdl.psthleft   = psthHdl(2);
    hdl.psthbottom = psthHdl(1);
else
    xlabel('time (sec)')
    ylabel('time (sec')
...
if strcmp(cfg.colorbar,'yes')
    ...
    hdl.colorbarHdl = colorbar; % create a colorbar
    
    % create a position vector and reset the position, it should be alligned right of JPSTH
    startPos = [pos(1) + 0.96 * origPos(3), pos(2) + 0.25 * pos(4)];
    sizePos  = [         0.04 * origPos(3),          0.75 * pos(4)];
    set(hdl.colorbarHdl, 'Position', [startPos sizePos])
    
    % set the text, try all the possible configurations
    try    isNormalized = strcmp(jpsth.cfg.normalization,'yes');
    catch, isNormalized = 0;
    end
    
    try
        unit = jpsth.cfg.previous.outputunit;
        if     strcmp(unit,      'rate'); isRate = 1;
        elseif strcmp(unit,'spikecount'); isRate = 2;
        end
    catch
        isRate = 3;
    end
    
    if isNormalized;    colorbarLabel = 'Normalized Joint Activity';
    elseif isRate == 1; colorbarLabel = 'Joint Firing Rate (spikes^2/sec^2)';
    elseif isRate == 2; colorbarLabel = 'Joint Spike Count (spikes^2)';
    else,               colorbarLabel = 'Joint Peristimulus Activity';
    end
    ...
    set(get(hdl.colorbarHdl,'YLabel'),'String',colorbarLabel)
end
...
barplotLim = {};
if ~strcmp(cfg.barplot,'no')
    barplotLim{1} = get(ax(2),'YLim');
    barplotLim{2} = get(ax(3),'XLim');
end
...
set(zoom,'ActionPostCallback',{@mypostcallback,ax,cfg.latency,barplotLim});
set(pan, 'ActionPostCallback',{@mypostcallback,ax,cfg.latency,barplotLim});
...
if currentAxes == ax(1)
    ...
    % reset the x and y limits  to the shared limits
